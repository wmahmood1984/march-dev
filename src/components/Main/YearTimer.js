...
export default function YearlyTimer(props) {
    const [seconds, setSeconds] = useState(60);
    const stakeTime = Number(props.stakeTime)
...
//    var SecondsRemaining = Math.trunc((differenceTimeinSeconds )-(MonthsRemaining*30*24*60*60)-(DaysRemaining*24*60*60)-(HoursRemaining*60*60)-(MinutesRemaining*60))
    ...
    return (
            ...
            <div>
            {stakeTime !== 0 ? <div>
            <span className="custom">{MonthsRemaining} Months     </span>
            <span>{HoursRemaining} Days     </span>
